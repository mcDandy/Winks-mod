...
import static cz.mcDandy.winksmod.Size.createPatternHelper;
...
public class ModTeleporter implements ITeleporter {
     ...
     */
    public void makePortal(Entity entity) {
            ...
            for (int j7 = -1; j7 <= 1; ++j7) {
                for (int i8 = 1; i8 < 3; ++i8) {
                    for (int i9 = -1; i9 < 3; ++i9) {
                        ...
                        world.setBlockState(mutable, flag ? FRAME_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
                    }
                }
            }
        }
        ...
        for (int fWidth = -1; fWidth < 3; ++fWidth) {
            for (int fHeight = -1; fHeight < 4; ++fHeight) {
                if (fWidth == -1 || fWidth == 2 || fHeight == -1 || fHeight == 3) {
                    ...
                    world.setBlockState(mutable, FRAME_BLOCK.getDefaultState(), 3);
                }
            }
        }

        BlockState portal = PORTAL_BLOCK.getDefaultState().with(AXIS, xAxis == 0 ? Direction.Axis.Z : Direction.Axis.X);
